...
import BasketBrewCartItemComponent from '../components/BasketBrewCartItemComponent';
...
export default function () {
  ...
  const handleOrder = () => {
    const destinationScreen = cart.length
      ? 'BasketBrewCartSuccessScreen'
      : 'BasketBrewHomeScreen';
        ...
        <>
          <View style={{height: height * 0.7}}>
            <ScrollView style={styles.flex} contentContainerStyle={styles.main}>
              {cart.map((item, index) => (
                <BasketBrewCartItemComponent item={item} key={index} />
              ))}
            </ScrollView>
          </View>
        </>
